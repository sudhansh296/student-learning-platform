...
.uce-detail pre{background:#1a1a1a;color:#4ade80;padding:12px;border-radius:8px;font-size:12px;overflow-x:auto;margin:0;}`,
      js: `var cases = [
  {
    ...
    cmd: 'SET cache:user:42 \\'{\"name\":\"Alice\",\"email\":\"a@b.com\"}\\' EX 300\ GET cache:user:42'
  },
  {
    ...
    cmd: 'SET session:xyz123 \\'{\"userId\":42,\"role\":\"admin\"}\\' EX 1800\ GET session:xyz123\ DEL session:xyz123'
  },
  {
    ...
    cmd: 'INCR rate:192.168.1.1\ EXPIRE rate:192.168.1.1 60\ # Check if over limit: GET rate:192.168.1.1'
  },
  {
    ...
    cmd: 'ZADD leaderboard 4200 \"alice\"\ ZADD leaderboard 3800 \"bob\"\ ZREVRANGE leaderboard 0 9 WITHSCORES'
  },
  {
    ...
    cmd: '# Subscriber (runs in separate connection)\ SUBSCRIBE notifications\ \ # Publisher\ PUBLISH notifications \"New order placed: #1042\"'
  },
  {
    ...
    cmd: '# Producer\ LPUSH jobs:email \\'{\"to\":\"user@example.com\",\"subject\":\"Welcome\"}?\\'\ \ # Worker (blocks until item available)\ BRPOP jobs:email 0'
  }
];
