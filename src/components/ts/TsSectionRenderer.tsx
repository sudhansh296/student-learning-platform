...
<style>*{box-sizing:border-box}body{margin:0;padding:14px;font-family:system-ui,sans-serif;font-size:15px;line-height:1.6}${c}</style>
</head><body><div id="output"></div><script>
const __logs=[];const __orig=console.log;
console.log=(...a)=>{__orig(...a);__logs.push(a.map(x=>typeof x==='object'?JSON.stringify(x,null,2):String(x)).join(' '));};
try{${j}}catch(e){document.getElementById('output').innerHTML+='<p style="color:red;font-family:monospace;font-size:13px">❌ '+e.message+'</p>';}
if(__logs.length){const p=document.createElement('pre');p.style.cssText='background:#0d1117;color:#3fb950;padding:12px;border-radius:8px;font-size:13px;margin-top:8px;white-space:pre-wrap;word-break:break-all';p.textContent=__logs.join('\\n');document.body.appendChild(p);}
</script></body></html>`;
  }, []);
...
export function TsSectionRenderer({ sections }: { sections: TsSection[] }) {
  return (
          ...
          case 'heading':
            return (
              <h2 key={i}
                ...
                style={{ color: 'var(--text)', borderBottom: '2px solid #3178c6' }}>
                {s.content}
              </h2>
            );
              ...
              <div key={i} className="my-6">
                {s.title && (
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-1 h-5 rounded-full" style={{ background: '#3178c6' }} />
                    <p className="text-[11px] font-extrabold uppercase tracking-widest" style={{ color: '#3178c6' }}>
                      Example — {s.title}
                    </p>
                  </div>
                )}
                {s.content && (
                  <p className="text-[14px] leading-relaxed mb-3 pl-3" style={{ color: 'var(--text-2)', borderLeft: '3px solid #3178c6' }}>
                    {s.content}
                  </p>
                )}
          ...
          case 'tryit':
            return (
              <div key={i} className="my-6">
                ...
                <TsPlayground js={s.js || ''} css={s.css || ''} title={s.title} />
              </div>
            );
                ...
                <ul className="space-y-2">
                  {s.items?.map((item, j) => (
                    <li key={j} className="flex items-start gap-2.5 text-[14px] leading-relaxed" style={{ color: 'var(--text-2)' }}>
                      <span className="mt-2 w-1.5 h-1.5 rounded-full shrink-0" style={{ background: '#3178c6' }} />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
          ...
          case 'table':
            return (
              <div key={i} className="my-5 overflow-x-auto">
                {s.title && <p className="text-sm font-semibold mb-2" style={{ color: 'var(--text)' }}>{s.title}</p>}
