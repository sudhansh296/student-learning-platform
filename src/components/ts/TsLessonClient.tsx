...
import type { TsLesson } from '@/data/ts-curriculum';
import { TsSectionRenderer } from './TsSectionRenderer';
...
export function TsLessonClient({ lesson, allLessons, chapters, prev, next }: Props) {
    ...
    <aside className={`w-64 shrink-0 ${sidebarOpen ? 'fixed inset-y-0 left-0 z-50 bg-background border-r border-border shadow-xl overflow-y-auto pt-4' : 'hidden lg:block'}`}>
      ...
      <div className="sticky top-20 overflow-y-auto max-h-[calc(100vh-5rem)] pb-10 px-3">
        <div className="mb-4 px-2">
          <Link href="/learn/typescript" className="flex items-center gap-2">
            <span className="text-xl">🔷</span>
            <span className="font-bold text-sm text-foreground">TypeScript Tutorial</span>
          </Link>
        </div>
        {chapters.map(ch => {
            ...
            <div key={ch.id} className="mb-4">
              ...
              <ul className="space-y-0.5">
                {chLessons.map(l => {
                  ...
                  return (
                    <li key={l.id}>
                      <Link href={`/learn/typescript/${l.slug}`}
                        onClick={() => setSidebarOpen(false)}
                        className={`block px-3 py-2 rounded-lg text-sm transition-colors ${
                          isActive
                            ? 'bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 font-semibold border border-blue-200 dark:border-blue-800'
    ...
    <div className="max-w-screen-xl mx-auto px-4 lg:px-6">
      <div className="flex gap-8 py-8">
        ...
        <main className="flex-1 min-w-0">
          ...
          <TsSectionRenderer sections={lesson.sections} />
