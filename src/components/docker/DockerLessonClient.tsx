...
import type { DockerLesson } from '@/data/docker-curriculum';
import { DockerChapters } from '@/data/docker-curriculum';
import { DockerSectionRenderer } from './DockerSectionRenderer';
...
interface Props {
  lesson: DockerLesson;
  allLessons: DockerLesson[];
}

export default function DockerLessonClient({ lesson, allLessons }: Props) {
  ...
  const sidebar = (
    <aside className={`w-64 shrink-0 ${sidebarOpen ? 'fixed inset-y-0 left-0 z-50 overflow-y-auto pt-4 shadow-xl' : 'hidden lg:block'}`}
      ...
      <div className="sticky top-20 overflow-y-auto max-h-[calc(100vh-5rem)] pb-10 px-3">
        <Link href="/learn/docker/01-introduction" className="flex items-center gap-2 px-2 mb-5">
          <span className="text-xl">🐳</span>
          <span className="font-extrabold text-sm" style={{ color: 'var(--text)' }}>Docker Tutorial</span>
        </Link>
        {DockerChapters.map(ch => {
            ...
            <div key={ch.id} className="mb-4">
              ...
              <ul className="space-y-0.5">
                {chLessons.map(l => {
                  ...
                  return (
                    <li key={l.id}>
                      <Link href={`/learn/docker/${l.slug}`} onClick={() => setSidebarOpen(false)}
      ...
      <div className="flex gap-0 py-0">
        ...
        <main className="flex-1 min-w-0 py-8 lg:pl-10">
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden flex items-center gap-2 mb-5 px-3 py-2 rounded-lg text-sm"
            ...
            <Menu className="w-4 h-4" /> All Docker Topics
          </button>
          <Breadcrumb items={[{ label: 'Learn', href: '/learn' }, { label: 'Docker', href: '/learn/docker/01-introduction' }, { label: lesson.title }]} />
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: '#eff8ff', color: BRAND, border: `1px solid ${BRAND}` }}>
                {DockerChapters.find(c => c.id === lesson.chapter)?.title}
              </span>
