...
function CopyCodeBtn({ code }: { code: string }) {
  ...
  return (
    <button onClick={async () => { await navigator.clipboard.writeText(code).catch(() => {}); setCopied(true); setTimeout(() => setCopied(false), 1800); }}
      ...
      style={{ background: '#F29111' }}>
      {copied ? <><Check className="w-3 h-3 text-white" /> Copied!</> : <><Copy className="w-3 h-3" /> Copy Code</>}
    </button>
  );
}

function SqlPlayground({ js, css, title }: { js: string; css: string; title?: string }) {
      ...
      <div className={`my-5 rounded-xl overflow-hidden border border-[#30363d] bg-[#0d1117] ${fullscreen ? 'fixed inset-4 z-[200] flex flex-col' : ''}`}>
        <div className="flex items-center justify-between px-4 py-2 bg-[#161b22] border-b border-[#30363d]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] font-mono font-semibold text-white">SQL Simulation</span>
        ...
        <div className={`flex flex-col md:flex-row ${fullscreen ? 'flex-1 min-h-0' : ''}`}>
          ...
          <div className="flex flex-col w-full md:w-1/2 bg-white" style={{ minHeight: 200 }}>
            <div className="flex items-center justify-between px-3 py-1.5 bg-[#f0f2f4] border-b border-[#d0d7de]">
              ...
              <button onClick={run} className="flex items-center gap-1.5 px-3 py-1 text-white text-[11px] font-semibold rounded" style={{ background: '#F29111' }}>
                <Play className="w-3 h-3" /> Run
              </button>
            </div>
            {blobUrl
              ? <iframe src={blobUrl} className="flex-1 border-0 w-full bg-white" sandbox="allow-scripts allow-same-origin" title="SQL Preview" />
    ...
    <div className="space-y-1 max-w-[72ch]">
      {sections.map((s, i) => {
        switch (s.type) {
          ...
          case 'heading': return <h2 key={i} id={(s.content||'').toLowerCase().replace(/\s+/g,'-').replace(/[^\w-]/g,'')} className="text-xl font-extrabold mt-10 mb-4 pb-2 scroll-mt-20" style={{ color: 'var(--text)', borderBottom: '2px solid #F29111' }}>{s.content}</h2>;
          case 'example': return (
            <div key={i} className="my-6">
              {s.title && <div className="flex items-center gap-2 mb-2"><div className="w-1 h-5 rounded-full" style={{ background: '#F29111' }} /><p className="text-[11px] font-extrabold uppercase tracking-widest" style={{ color: '#F29111' }}>Example -- {s.title}</p></div>}
              {s.content && <p className="text-[14px] leading-relaxed mb-3 pl-3" style={{ color: 'var(--text-2)', borderLeft: '3px solid #F29111' }}>{s.content}</p>}
          ...
          case 'tryit': return (
            <div key={i} className="my-6">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-1 h-5 rounded-full" style={{ background: '#F29111' }} />
                <p className="text-xs font-extrabold uppercase tracking-widest" style={{ color: '#F29111' }}>Try It Yourself{s.title ? ` -- ${s.title}` : ''}</p>
              </div>
              <SqlPlayground js={s.js || ''} css={s.css || ''} title={s.title} />
            </div>
          );
