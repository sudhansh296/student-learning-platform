...
import type { RedisLesson } from '@/data/redis-curriculum';
import { RedisChapters } from '@/data/redis-curriculum';
import { RedisSectionRenderer } from './RedisSectionRenderer';
...
interface Props {
  lesson: RedisLesson;
  allLessons: RedisLesson[];
}

export default function RedisLessonClient({ lesson, allLessons }: Props) {
  ...
  const sidebar = (
    <aside className={`w-64 shrink-0 ${sidebarOpen ? 'fixed inset-y-0 left-0 z-50 overflow-y-auto pt-4 shadow-xl' : 'hidden lg:block'}`}
      ...
      <div className="sticky top-20 overflow-y-auto max-h-[calc(100vh-5rem)] pb-10 px-3">
        <Link href="/learn/redis/01-introduction" className="flex items-center gap-2 px-2 mb-5">
          <span className="text-xl">🔴</span>
          <span className="font-extrabold text-sm" style={{ color: 'var(--text)' }}>Redis Tutorial</span>
        </Link>
        {RedisChapters.map(ch => {
            ...
            <div key={ch.id} className="mb-4">
              ...
              <ul className="space-y-0.5">
                {chLessons.map(l => {
                  ...
                  return (
                    <li key={l.id}>
                      <Link href={`/learn/redis/${l.slug}`} onClick={() => setSidebarOpen(false)}
                        className="block px-3 py-2 rounded-lg text-[13px] transition-all"
                        style={{ background: isActive ? '#fff0ef' : 'transparent', color: isActive ? BRAND : 'var(--text-2)', fontWeight: isActive ? '700' : '500', borderLeft: isActive ? `3px solid ${BRAND}` : '3px solid transparent' }}>
                        {l.title}
                      </Link>
                    </li>
                  );
      ...
      <div className="flex gap-0 py-0">
        ...
        <main className="flex-1 min-w-0 py-8 lg:pl-10">
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden flex items-center gap-2 mb-5 px-3 py-2 rounded-lg text-sm"
            ...
            <Menu className="w-4 h-4" /> All Redis Topics
          </button>
          <Breadcrumb items={[{ label: 'Learn', href: '/learn' }, { label: 'Redis', href: '/learn/redis/01-introduction' }, { label: lesson.title }]} />
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: '#fff0ef', color: BRAND, border: `1px solid ${BRAND}` }}>
                {RedisChapters.find(c => c.id === lesson.chapter)?.title}
              </span>
          ...
          <RedisSectionRenderer sections={lesson.sections} />
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {prevLesson ? (
              <Link href={`/learn/redis/${prevLesson.slug}`} className="group flex items-center gap-3 p-4 rounded-2xl transition-all hover:-translate-y-0.5" style={{ border: '1px solid var(--line)', background: 'var(--card)' }}>
            ...
            {nextLesson ? (
              <Link href={`/learn/redis/${nextLesson.slug}`} className="group flex items-center justify-end gap-3 p-4 rounded-2xl transition-all hover:-translate-y-0.5 text-right" style={{ border: `1px solid ${BRAND}`, background: '#fff0ef' }}>
