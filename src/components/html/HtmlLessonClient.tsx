...
interface Props {
  lesson: HtmlLesson;
  allLessons: HtmlLesson[];
  chapters: { id: string; title: string; icon: string }[];
  prev: HtmlLesson | null;
  next: HtmlLesson | null;
}
...
export function HtmlLessonClient({ lesson, allLessons, chapters, prev, next }: Props) {
  ...
  const sidebar = (
    <aside className={`w-64 shrink-0 ${sidebarOpen ? 'fixed inset-y-0 left-0 z-50 overflow-y-auto pt-4 shadow-xl' : 'hidden lg:block'}`}
      ...
      <div className="sticky top-20 overflow-y-auto max-h-[calc(100vh-5rem)] pb-10 px-3">
        {/* Brand */}
        <Link href="/html" className="flex items-center gap-2 px-2 mb-5">
          <span className="text-xl">🌐</span>
          <span className="font-extrabold text-sm" style={{ color: 'var(--text)' }}>HTML Tutorial</span>
        </Link>
      ...
      <div className="flex gap-0 py-0">
        ...
        <main className="flex-1 min-w-0 py-8 lg:pl-10">
          ...
          <button onClick={() => setSidebarOpen(true)}
            className="lg:hidden flex items-center gap-2 mb-5 px-3 py-2 rounded-lg text-sm transition-colors"
            style={{ border: '1px solid var(--line)', color: 'var(--text-2)', background: 'var(--bg)' }}>
            <Menu className="w-4 h-4" /> All HTML Topics
          </button>
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-3)' }}>
                ...
                {chapters.find(c => c.id === lesson.chapter)?.title}
              </span>
            </div>
          </div>

          {/* Content */}
          <HtmlSectionRenderer sections={lesson.sections} />
