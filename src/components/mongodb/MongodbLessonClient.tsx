...
import type { MongodbLesson } from '@/data/mongodb-curriculum';
import { mongodbChapters } from '@/data/mongodb-curriculum';
import { MongodbSectionRenderer } from './MongodbSectionRenderer';
...
interface Props {
  lesson: MongodbLesson;
  allLessons: MongodbLesson[];
}

export default function MongodbLessonClient({ lesson, allLessons }: Props) {
  ...
  const BRAND = '#00ED64';
  const BRAND_DARK = '#001E2B';

  const sidebar = (
    <aside
      className={`w-64 shrink-0 ${sidebarOpen ? 'fixed inset-y-0 left-0 z-50 overflow-y-auto pt-4 shadow-xl' : 'hidden lg:block'}`}
    ...
    >
      ...
      <div className="sticky top-20 overflow-y-auto max-h-[calc(100vh-5rem)] pb-10 px-3">
        <Link href="/learn/mongodb/introduction" className="flex items-center gap-2 px-2 mb-5">
          <span className="text-xl">🍃</span>
          <span className="font-extrabold text-sm" style={{ color: 'var(--text)' }}>MongoDB Tutorial</span>
        </Link>

        {mongodbChapters.map(ch => {
          const chLessons = allLessons.filter(l => l.chapter === ch.id);
          if (!chLessons.length) return null;
          return (
            <div key={ch.id} className="mb-4">
              <p className="text-[10px] font-extrabold uppercase tracking-widest px-2 mb-1.5"
      ...
      <div className="flex gap-0 py-0">
        ...
        <main className="flex-1 min-w-0 py-8 lg:pl-10">
          ...
          <button onClick={() => setSidebarOpen(true)}
            ...
            <Menu className="w-4 h-4" /> All MongoDB Topics
          </button>

          <Breadcrumb items={[
            { label: 'Learn', href: '/learn' },
            { label: 'MongoDB', href: '/learn/mongodb/introduction' },
            { label: lesson.title },
          ]} />
          ...
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xs font-bold px-2.5 py-1 rounded-full"
                style={{ background: '#f0fff4', color: BRAND_DARK, border: `1px solid ${BRAND}` }}>
                {mongodbChapters.find(c => c.id === lesson.chapter)?.title}
              </span>
            ...
            <p className="text-[15px] leading-relaxed mb-4 max-w-2xl" style={{ color: 'var(--text-2)' }}>
              {lesson.description}
            </p>
          ...
          {/* Content */}
          <MongodbSectionRenderer sections={lesson.sections} />
            ...
            {prevLesson ? (
              <Link href={`/learn/mongodb/${prevLesson.slug}`}
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm font-semibold truncate" style={{ color: 'var(--text)' }}>{prevLesson.title}</p>
                </div>
              </Link>
            ) : <div />}

            {nextLesson ? (
              <Link href={`/learn/mongodb/${nextLesson.slug}`}
                className="group flex items-center justify-end gap-3 p-4 rounded-2xl transition-all hover:-translate-y-0.5 text-right"
                style={{ border: `1px solid ${BRAND}`, background: '#f0fff4' }}>
                <div className="min-w-0">
                  <p className="text-xs" style={{ color: 'var(--text-3)' }}>Next</p>
                  <p className="text-sm font-semibold truncate" style={{ color: BRAND_DARK }}>{nextLesson.title}</p>
                </div>
                <ChevronRight className="w-5 h-5 shrink-0" style={{ color: BRAND_DARK }} />
              </Link>
            ) : <div />}
          </div>
        </main>
