...
import type { PostgresqlLesson } from '@/data/postgresql-curriculum';
import { PostgresqlChapters } from '@/data/postgresql-curriculum';
import { PostgresqlSectionRenderer } from './PostgresqlSectionRenderer';
...
interface Props {
  lesson: PostgresqlLesson;
  allLessons: PostgresqlLesson[];
}

export default function PostgresqlLessonClient({ lesson, allLessons }: Props) {
  ...
  const sidebar = (
    <aside className={`w-64 shrink-0 ${sidebarOpen ? 'fixed inset-y-0 left-0 z-50 overflow-y-auto pt-4 shadow-xl' : 'hidden lg:block'}`}
      ...
      <div className="sticky top-20 overflow-y-auto max-h-[calc(100vh-5rem)] pb-10 px-3">
        <Link href="/learn/postgresql/01-introduction" className="flex items-center gap-2 px-2 mb-5">
          <span className="text-xl">🐘</span>
          <span className="font-extrabold text-sm" style={{ color: 'var(--text)' }}>PostgreSQL Tutorial</span>
        </Link>
        {PostgresqlChapters.map(ch => {
            ...
            <div key={ch.id} className="mb-4">
              ...
              <ul className="space-y-0.5">
                {chLessons.map(l => {
                  ...
                  return (
                    <li key={l.id}>
                      <Link href={`/learn/postgresql/${l.slug}`} onClick={() => setSidebarOpen(false)}
                        className="block px-3 py-2 rounded-lg text-[13px] transition-all"
                        style={{ background: isActive ? '#eff6ff' : 'transparent', color: isActive ? BRAND : 'var(--text-2)', fontWeight: isActive ? '700' : '500', borderLeft: isActive ? `3px solid ${BRAND}` : '3px solid transparent' }}>
                        {l.title}
                      </Link>
                    </li>
                  );
      ...
      <div className="flex gap-0 py-0">
        ...
        <main className="flex-1 min-w-0 py-8 lg:pl-10">
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden flex items-center gap-2 mb-5 px-3 py-2 rounded-lg text-sm"
            ...
            <Menu className="w-4 h-4" /> All PostgreSQL Topics
          </button>
          <Breadcrumb items={[{ label: 'Learn', href: '/learn' }, { label: 'PostgreSQL', href: '/learn/postgresql/01-introduction' }, { label: lesson.title }]} />
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: '#eff6ff', color: BRAND, border: `1px solid ${BRAND}` }}>
                {PostgresqlChapters.find(c => c.id === lesson.chapter)?.title}
              </span>
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {nextLesson ? (
              <Link href={`/learn/postgresql/${nextLesson.slug}`} className="group flex items-center justify-end gap-3 p-4 rounded-2xl transition-all hover:-translate-y-0.5 text-right" style={{ border: `1px solid ${BRAND}`, background: '#eff6ff' }}>
